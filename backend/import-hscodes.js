...
async function importHsCodes() {
  ...
  try {
    ...
    console.log(`Found ${hsCodes.length} HS codes to import`);
    ...
    for (let i = 0; i < hsCodes.length; i += batchSize) {
      ...
      console.log(`Importing batch ${Math.floor(i/batchSize) + 1} of ${Math.ceil(hsCodes.length/batchSize)}`);
      
      // Use bulkCreate with updateOnDuplicate to insert or update existing codes
      await HSCode.bulkCreate(batch, {
        updateOnDuplicate: ['description', 'section', 'chapter', 'updatedAt']
      });
    }
    
    const count = await HSCode.count();
    console.log(`Successfully imported HS codes. Total HS codes in database: ${count}`);
